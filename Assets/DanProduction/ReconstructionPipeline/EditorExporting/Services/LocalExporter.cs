...
public static class LocalExporter
{
    ...
    static bool ResolveVersion(ExportContext context)
    {
        ...
        int latestVer = latestFolder == null
            ? 0: GetVersionNumber(latestFolder.Name);
    ...
    public static void ExportLevel(LevelMetadata exportingMetadata, Texture2D thumbnail, ExportSettings exportSettings)
    {
        ...
        //Report any problems
        ValidateLevelDirectory(targetDirectory);
        
        //ExportContext export = new ExportContext(exportingMetadata, exportSettings, targetDirectory);
        //Resolve any settings and final changes
        //ResolveExportSettings(export);
        
        //Export to disk
        //string extractionPath = Path.Combine(targetDirectory.FullName, $"v{export.metadata.ContentVersion:D3}");
        //Directory.CreateDirectory(extractionPath);

        //WriteMetadata(extractionPath, export.metadata);
        //WriteLevelData
        //WriteThumbnail(extractionPath, thumbnail);
    ...
    static void ValidateLevelDirectory(DirectoryInfo levelDirectory)
    {
    ...
    /*
    static void ResolveExportSettings(ExportContext exportContext)
    {
        //resolve versions
        if (exportContext.settings.AutomaticVersioning)
        {
            int resolvedVersion = 1;
            //find version folders
            DirectoryInfo latestFolder = exportContext.targetDir.GetDirectories()
                .Where(folder => GetVersionNumber(folder.Name) >= 0)
                .OrderByDescending(folder => GetVersionNumber(folder.Name)).FirstOrDefault();
            
            //if available, increment version
            if (latestFolder != null)
            {
                resolvedVersion = GetVersionNumber(latestFolder.Name) + 1;
            }
            
            //exportContext.metadata.SetContentVersion(resolvedVersion);
        }
    }
    */
    #endregion

    #region Export procedures

    static void WriteMetadata(string extractionPath, LevelMetadata metadata)
    {
        string json = JsonConvert.SerializeObject
            (metadata, Formatting.Indented);
        string metadataPath = Path.Combine
            (extractionPath, $"metadata.json");
        File.WriteAllText(metadataPath, json);
    }

    static void WriteThumbnail(string extractionPath, Texture2D thumbnail, bool shipping = false)
    {
        string thumbnailPath = Path.Combine
            (extractionPath, $"thumbnail.png");
